...
ALTER VIEW XGSG_LPLUSINVOICEDETAIL AS
...
alter View XGSG_Invoice_Detail as
...
Alter View XGSG_HomeDepotInvoice as
...
Select [Lease Number]
	...
	, [Invoice Number]
	, [PropertyTax]
	, Round([SalesTax%] * rent,2) as [Sales Tax]
	, Rent
From(
Select  isnull(invdtl_lease_num,'') [Lease Number]
	, Isnull(Equip_Seq_Num,'') [Asset Number]
	, isnull(equip_serial_num,'') [Serial Number]
	, Right( '00' + convert(varchar,DatePart(Month,Invdtl_Due_Date)),2) [Month]
	, datepart(Year,Invdtl_Due_Date) [Year]
	, datepart(day,Invdtl_Due_Date) [Day]
	, CASE 
		When isnull(DatePart(Month,Invdtl_Due_Date),'') = '1' then 'January'
		When isnull(DatePart(Month,Invdtl_Due_Date),'') = '2' then 'February'
		When isnull(DatePart(Month,Invdtl_Due_Date),'') = '3' then 'March'
		WHEN isnull(DatePart(Month,Invdtl_Due_Date),'') = '4' THEN 'April'
		WHEN isnull(DatePart(Month,Invdtl_Due_Date),'') = '5' THEN 'May'
		WHEN isnull(DatePart(Month,Invdtl_Due_Date),'') = '6' THEN 'June'
		WHEN isnull(DatePart(Month,Invdtl_Due_Date),'') = '7' THEN 'July'
		WHEN isnull(DatePart(Month,Invdtl_Due_Date),'') = '8' THEN 'August'
		WHEN isnull(DatePart(Month,Invdtl_Due_Date),'') = '9' THEN 'September'
		WHEN isnull(DatePart(Month,Invdtl_Due_Date),'') = '10' THEN 'October'
		WHEN isnull(DatePart(Month,Invdtl_Due_Date),'') = '11' THEN 'November'
		WHEN isnull(DatePart(Month,Invdtl_Due_Date),'') = '12' THEN 'December'
		ELSE '' END AS Month2
	...
	, CONVERT(DATE, Invdtl_Due_Date) [Invoice Due Date]
	, Invdtl_cust_id_num [Customer #]
	, equip_vend_po_num as SGLN
	, invdtl_inv_num [Invoice Number]
	, [PropertyTax]
	, CASE
		When InvRentAmt = 0 then 0
		When InvRentAmt <> 0 then [SalesTax]/InvRentAmt
		end  [SalesTax%] 
	, equip_rent RENT
from lplusequipmentvw  E
Join (
Select invdtl_lease_num
	,Invdtl_cust_id_num
	,invdtl_inv_num
...
Select invdtl_lease_num
	...
	,CASE INVDTL_TRAN_CODE
		WHEN 'SALETAX'	THEN INVDTL_AMOUNT
		ELSE 0 END AS [SalesTax]
from LPlusOpenInvoiceDetailVW
where invdtl_cust_id_num = '21861'
)X
group by invdtl_lease_num
	,Invdtl_cust_id_num
	,invdtl_inv_num
	...
	on Y.invdtl_lease_num = E.Equip_lease_num 
	)z
